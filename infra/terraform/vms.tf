locals {
  ...
  worker_vms = {
    # extra_disk_size: extra virtual disk for Longhorn storage (0 = use main disk only)
  ...
  # Unique Proxmox nodes for worker VMs (need separate image with iscsi-tools)
  worker_proxmox_nodes = toset([for v in local.worker_vms : v.proxmox_node])
...
# Worker-specific nocloud image with iscsi-tools + util-linux-tools for Longhorn
...
resource "proxmox_virtual_environment_download_file" "talos_worker_image" {
  for_each = local.worker_proxmox_nodes

  content_type            = "iso"
  datastore_id            = var.iso_datastore
  node_name               = each.key
  url                     = "https://factory.talos.dev/image/53513e54bb39202f35694412577a6bc53d484744d35a126e5d42ef34785c0d83/${var.talos_version}/nocloud-amd64.raw.zst"
  file_name               = "talos-${var.talos_version}-worker-nocloud-amd64.img"
...
resource "proxmox_virtual_environment_vm" "worker" {
  ...
  # Worker nocloud image includes iscsi-tools + util-linux-tools for Longhorn
  disk {
    datastore_id = var.vm_datastore
    interface    = "scsi0"
    size         = each.value.disk_size
    file_id      = proxmox_virtual_environment_download_file.talos_worker_image[each.value.proxmox_node].id
    file_format  = "raw"
    discard      = "on"
  }
